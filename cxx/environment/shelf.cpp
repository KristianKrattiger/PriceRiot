...
PickResult try_pick(ShelfSide &side, const SideBand &band, std::uint8_t stall_idx,
    ...
    if (st.bay.face < bay->face_count) {
        ...
        auto res =
            face.take_one(st.bay.slot < face.slot_count ? std::optional<std::uint8_t>(st.bay.slot)
                                                        : std::nullopt);
        if (res.second) {
            r.sku_id = res.first;
            r.success = true;
...
void set_stall_blocked(SideBand &band, ShelfSide &side, std::uint8_t stall_idx, bool on) noexcept {
    ...
    if (Bay *b = side.try_get(band.stalls[stall_idx].bay.bay))
        b->blocked = on;
}
...
std::optional<std::uint8_t> select_stall_nearest_free(const SideBand &band,
    ...
    if (best_idx >= 0)
        return static_cast<std::uint8_t>(best_idx);
    return std::nullopt;
}
