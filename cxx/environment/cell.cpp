...
EdgeCell::EdgeCell(int id, double length_m, double personal_radius_m)
    : id_(id),
      d_cell_(length_m < 0.0 ? 0.0 : length_m),
      r_personal_(personal_radius_m < 0.0 ? 0.0 : personal_radius_m)
{
...
EdgeCell::select_stall_exact(const SideBand& band, std::uint16_t bay) noexcept {
    if (bay < band.count) {
        const Stall& s = band.stalls[bay];
        if (s.free() && !s.blocked) return static_cast<std::uint8_t>(bay);
    }
    return std::nullopt;
}

std::optional<std::uint8_t>
EdgeCell::select_stall_nearest_free(const SideBand& band, std::uint16_t target_bay) noexcept {
    if (band.count == 0) return std::nullopt;
    if (target_bay >= band.count) target_bay = band.count - 1;
...
bool EdgeCell::try_peel_to_stall(SideBand& band, std::uint8_t stall_idx, std::uint32_t agent_id) noexcept {
    if (stall_idx >= band.count) return false;
    Stall& s = band.stalls[stall_idx];
    if (!s.free() || s.blocked) return false;
    s.occ = agent_id;
    s.t_since_occupied = 0.f;
    return true;
}

PickResult EdgeCell::try_pick(const ShelfSide& inv_const,
                              SideBand& band,
    ...
    Stall& s = band.stalls[stall_idx];
    if (!s.occ.has_value() || s.occ.value() != agent_id) return r;

    ShelfSide& inv = const_cast<ShelfSide&>(inv_const);
    if (s.bay.bay >= inv.bay_count) return r;

    Bay& bay = inv.bays[s.bay.bay];
    if (bay.blocked) return r;
    ...
    if (r.success) r.time_cost_s = pick_time_s.value_or(0.f);
    return r;
}

bool EdgeCell::try_merge_from_stall(SideBand& band, std::uint8_t stall_idx, std::uint32_t agent_id) noexcept {
    if (stall_idx >= band.count) return false;
    Stall& s = band.stalls[stall_idx];
    if (!s.occ.has_value() || s.occ.value() != agent_id) return false;
    s.occ.reset();
    s.t_since_occupied = 0.f;
    return true;
}

void EdgeCell::set_stall_blocked(SideBand& band, const ShelfSide& inv_const,
                                 std::uint8_t stall_idx, bool on) noexcept {
    if (stall_idx >= band.count) return;
    Stall& s = band.stalls[stall_idx];
    s.blocked = on;
    ShelfSide& inv = const_cast<ShelfSide&>(inv_const);
    if (stall_idx < inv.bay_count) inv.bays[stall_idx].blocked = on;
}
...
PickResult EdgeCell::pick_left(std::uint8_t stall_idx,
                               std::uint32_t agent_id,
                               std::optional<float> pick_time_s) noexcept {
    return try_pick(leftSide, leftBand, stall_idx, agent_id, pick_time_s);
}
PickResult EdgeCell::pick_right(std::uint8_t stall_idx,
                                std::uint32_t agent_id,
                                std::optional<float> pick_time_s) noexcept {
...
bool EdgeCell::merge_left(std::uint8_t stall_idx, std::uint32_t agent_id) noexcept {
    ...
    if (ok) {
        bool any = false;
        for (std::uint8_t i = 0; i < leftBand.count; ++i)  if (leftBand.stalls[i].occ)  { any = true; break; }
        for (std::uint8_t i = 0; i < rightBand.count && !any; ++i) if (rightBand.stalls[i].occ) { any = true; break; }
        if (!any) agent_present_ = false;
    }
    return ok;
}
