...
class CollisionManager {
  public:
    ...
    void setPhysicsWorld(const PhysicsWorld *world) {
        physicsWorld = world;
    }
    ...
    void getAvoidanceVector(double x, double z, double radius, double &avoidX, double &avoidZ,
                            double maxDistance = 2.0) const;

    // Check if position would collide with other agents
    bool wouldCollideWithAgents(double x, double z, double radius,
                                const Customer *excludeAgent = nullptr) const;
