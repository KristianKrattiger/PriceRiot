...
void PhysicsGenerator::generateShelfObstacles(const StoreGraph& graph, const StoreLayout& layout, PhysicsWorld& world) {
    ...
    for (const auto& edge : edges) {
        ...
        if (edgeGeo.shelfLeft < 0.01 && edgeGeo.shelfRight < 0.01) continue;
        ...
        if (edgeGeo.shelfLeft > 0.01) {
            ...
            AABB shelfObstacle(
                shelfStartX - shelfWidth / 2.0, shelfStartZ - shelfWidth / 2.0,
...
void PhysicsGenerator::generateBoundaries(const StoreLayout& layout, PhysicsWorld& world, double wallThickness) {
