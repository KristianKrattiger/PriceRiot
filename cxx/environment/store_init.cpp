...
template <typename T>
static T yaml_get(const YAML::Node& n, const char* key, const T& def) {
...
load_products_from_yaml(const YAML::Node& root, Catalog& catalog)
{
    ...
    if (!productsY || !productsY.IsSequence()) return totals_by_sku;

    for (const auto& p : productsY) {
        // ---- SKU (string or int) ----
        std::string sku_str;
        if (p["sku"]) {
            try { sku_str = std::to_string(p["sku"].as<int>()); }
            catch (...) { sku_str = p["sku"].as<std::string>(""); }
        }
        ...
        int sku_int;
        try { sku_int = std::stoi(sku_str); }
        catch (...) { sku_int = sku_string_to_int(sku_str); }
        ...
        const int total_units =
            p["total"] ? p["total"].as<int>()
                       : yaml_get<int>(p, "initial_stock_total", 0);
                             ...
                             std::unordered_map<std::uint32_t,int>& on_shelf_sum)
{
    ...
    if (!sideY || !sideY.IsMap()) return;

    // bay count (clamped to capacity)
    const int bay_count = std::max(0, yaml_get<int>(sideY, "bay_count", 0));
    side.bay_count = static_cast<std::uint8_t>(
        std::min<int>(bay_count, static_cast<int>(MAX_BAYS_PER_CELL))
    );

    const YAML::Node pog = sideY["planogram"];
    if (!pog || !pog.IsSequence()) return;
    ...
    for (const auto& rec : pog) {
        ...
        if (bay  < 0 || bay  >= static_cast<int>(MAX_BAYS_PER_CELL))  continue;
        if (face < 0 || face >= static_cast<int>(MAX_FACES_PER_BAY))  continue;
        if (slot < 0 || slot >= static_cast<int>(MAX_SLOTS_PER_FACE)) continue;

        // sku (string or int)
        std::string sku_str;
        if (rec["sku"]) {
            try { sku_str = std::to_string(rec["sku"].as<int>()); }
            catch (...) { sku_str = rec["sku"].as<std::string>(""); }
        }
        if (sku_str.empty()) continue;

        int sku_id;
        try { sku_id = std::stoi(sku_str); }
        catch (...) { sku_id = sku_string_to_int(sku_str); }
        ...
        on_shelf_sum[static_cast<std::uint32_t>(sku_id)] += std::max(0, qty_on_face);
    }
}

// ------- Backstock computation -------
void compute_and_load_backstock(const YAML::Node& root,
                                ...
                                InventoryPool& backroom)
{
    ...
    for (const auto& [sku_str, total] : totals_by_sku) {
        ...
        int visible = 0;
        if (auto it = on_shelf_sum.find(k); it != on_shelf_sum.end()) visible = it->second;
