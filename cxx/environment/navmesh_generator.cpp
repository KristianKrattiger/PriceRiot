...
void NavMeshGenerator::generateEdgePolygons(const Edge& edge, const EdgeGeometry& geom,
    ...
    // Use the edge's calculated clear width (already accounts for shelf protrusions and blocked areas)
...
void NavMeshGenerator::connectNodeEdgePolygons(const StoreGraph& graph,
...
NavPolygon NavMeshGenerator::createRectPolygon(int id, const std::vector<std::pair<double, double>>& corners) {
    std::vector<NavPolygon::Vertex> vertices;
    for (const auto& corner : corners) {
        vertices.emplace_back(corner.first, corner.second);
    }
...
std::vector<std::pair<double, double>> NavMeshGenerator::getEdgeWalkableCorners(
