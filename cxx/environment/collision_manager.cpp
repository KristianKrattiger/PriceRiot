...
void CollisionManager::getAvoidanceVector(double x, double z, double radius, double& avoidX, double& avoidZ, double maxDistance) const {
    avoidX = 0.0;
    avoidZ = 0.0;
...
bool CollisionManager::wouldCollideWithAgents(double x, double z, double radius, const Customer* excludeAgent) const {
    for (const auto& other : registeredAgents) {
        if (other.agent == excludeAgent) continue;
        
        if (circlesIntersect(x, z, radius, other.agent->getPosX(), other.agent->getPosZ(), other.radius)) {
            return true;
        }
    }
    return false;
...
bool CollisionManager::circlesIntersect(double x1, double z1, double r1, double x2, double z2, double r2) const {
    double dx = x2 - x1;
    double dz = z2 - z1;
    double distSq = dx * dx + dz * dz;
