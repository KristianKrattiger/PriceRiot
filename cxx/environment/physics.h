...
public:
    ...
    bool getCollisionInfo(const Circle& agent, double& normalX, double& normalZ, double& penetration) const;
    ...
    const std::vector<AABB>& getBoundaries() const { return boundaries; }

private:
    std::vector<AABB> obstacles;  // Store fixtures (shelves, displays, etc.)
    std::vector<AABB> boundaries;  // Store walls/boundaries
    
    bool checkCollisionWithObstacle(const Circle& agent, const AABB& obstacle, 
