...
namespace priceriot {
    ...
    struct BayFace {
        ...
        std::pair<std::uint32_t,bool> take_one(std::optional<std::uint8_t> preferred_slot = std::nullopt);
    };
    ...
    struct SideBand {
        ...
        [[nodiscard]] std::optional<std::uint8_t> find_free_for_bay(std::uint16_t bay_index) const noexcept;
