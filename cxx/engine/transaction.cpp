...
Transaction::Transaction(int customerId, int transactionId, std::vector<LineItem> items,
                         int satisfaction, std::string timestamp)
    : custID(customerId), transID(transactionId), items(std::move(items)),
      satisfaction(satisfaction), timestamp(std::move(timestamp)) {
...
// Getters
int Transaction::getCustID() const {
    return custID;
}
int Transaction::getTransID() const {
    return transID;
}
int Transaction::getSatisfaction() const {
    return satisfaction;
}
std::string Transaction::getTimestamp() const {
    return timestamp;
}
const std::vector<LineItem> &Transaction::getItems() const {
    return items;
}
double Transaction::getTotalSpent() const {
    return totalSpent;
}

// Setters
void Transaction::setCustID(int c) {
    custID = c;
}
void Transaction::setTransID(int id) {
    transID = id;
}
void Transaction::setSatisfaction(int s) {
    satisfaction = s;
}
void Transaction::setTimestamp(const std::string &ts) {
    timestamp = ts;
}
void Transaction::setItems(const std::vector<LineItem> &newItems) {
    items = newItems;
}
