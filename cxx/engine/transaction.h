...
struct LineItem {
    ...
    LineItem(int id, double ppu, int qty, std::string name)
        : id(id), pricePerUnit(ppu), quantity(qty), name(std::move(name)), total(ppu * qty) {}
};
...
class Transaction {
  public:
    ...
    Transaction(int custID_, int transID_, std::vector<LineItem> items_, int satisfaction_,
    ...
    static Transaction randomTransaction(std::vector<std::shared_ptr<Customer>> &customers,
