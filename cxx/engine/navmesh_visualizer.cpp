...
sf::Vector2f NavMeshVisualizer::worldToScreen(double x, double z) const {
    return sf::Vector2f(static_cast<float>(x) * pixelsPerMeter + offsetX,
...
void NavMeshVisualizer::drawConnections(sf::RenderWindow &window, const NavMesh &navmesh) {
    ...
    for (int i = 0; i < polygonCount; ++i) {
        ...
        for (int neighborIdx : neighbors) {
            if (neighborIdx < 0 || neighborIdx >= polygonCount)
                continue;
            ...
            // Draw line
            sf::Vertex line[2] = {sf::Vertex(centerScreen, connectionColor),
...
void NavMeshVisualizer::drawPath(sf::RenderWindow &window,
                                 const std::vector<std::pair<double, double>> &waypoints) {
    if (!showPaths || waypoints.size() < 2)
        return;
    ...
    for (size_t i = 0; i < waypoints.size() - 1; ++i) {
        ...
        sf::Vertex line[2] = {sf::Vertex(start, pathColor), sf::Vertex(end, pathColor)};
        window.draw(line, 2, sf::Lines);
    }
