...
#include "basket.h"
// CRITICAL FIX: Include Environment so StoreGraph is defined
...
// Default ctor
Customer::Customer()
  : id(0), annualIncome(0.0), totalSpent(0.0), averageSpend(0.0),
    loyaltyRating(0.0), weight(0.0), daysAsCust(0), numPurchases(0),
    numReturns(0), lastPurchaseInDays(0), age(0), familySize(1),
    promotionResponse(0.0), churn(false)
{}

// 4-arg ctor
Customer::Customer(const int id, double annualIncome_, int age_, std::string gender_)
  : id(id), annualIncome(annualIncome_), totalSpent(0.0), averageSpend(0.0),
    loyaltyRating(0.0), weight(0.0), daysAsCust(0), numPurchases(0),
    numReturns(0), lastPurchaseInDays(0), age(age_), familySize(1),
    gender(std::move(gender_)), promotionResponse(0.0), churn(false)
{}
...
void Customer::setBehavior(ICustomerBehavior* b) noexcept {
    delete behavior;
    behavior = b;
}

// --- MAIN UPDATE LOGIC ---
bool Customer::update(float dt, const StoreGraph& store, Basket& basket) {
    ...
    Decision d = behavior->decide(*this, ctx);

    // 3. Act on Decision
    switch (d.type) {
        case Decision::Wait:
            if (behaviorState.dwellTicks > 0) behaviorState.dwellTicks--;
            break;
        ...
        case Decision::Move:
            ...
            if (currentEdgeIndex != -1) {
                ...
                if (double len = store.edgeAt(currentEdgeIndex).getLength();
                    distOnEdge > len + 1.0) distOnEdge = len;
            }
            break;

        case Decision::SwitchEdge:
            currentEdgeIndex = d.targetId;
            distOnEdge = 0.0;
            behaviorState.lastShopCell = -1;
            break;

        case Decision::PickProduct:
            behaviorState.dwellTicks += static_cast<int>(d.duration * 60);
            break;
...
int recalcBasketSize(const std::shared_ptr<Customer>& customer, std::default_random_engine& rng) {
    ...
    double baseMean = 5.0 + (customer->getFamilySize() * 1.5);
    double targetMean = baseMean * customer->getBasketSizeMultiplier();
...
void Customer::setLoyaltyRating(const double rating) { loyaltyRating = rating; recalcWeight(); }
void Customer::setTotalSpent(const double totalSpent) { this->totalSpent = totalSpent; }
void Customer::setNumPurchases(const int numPurchases) { this->numPurchases = numPurchases; }
void Customer::setAverageSpend(const double avg) { averageSpend = avg; }
void Customer::setDwellTicks(int ticks) { behaviorState.dwellTicks = ticks; }

void Customer::updateLoyalty(const int satisfaction) {
    if (satisfaction > 5) loyaltyRating *= 1.1;
    else loyaltyRating /= 1.1;
...
std::shared_ptr<Customer> selectCustomer(std::vector<std::shared_ptr<Customer> > &customers,
    ...
    for (auto const& cptr : customers) weights.push_back(cptr->getWeight());
